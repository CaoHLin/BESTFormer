...
from quantized_utils import IRConv2d, IRConv1d
...
class MLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, drop=0.):
        ...
        self.mlp1_conv = IRConv2d(in_features, hidden_features, kernel_size=1, stride=1)
        self.mlp1_bn = nn.BatchNorm2d(hidden_features)

        self.mlp2_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy')
        self.mlp2_conv = IRConv2d(hidden_features, out_features, kernel_size=1, stride=1)
...
class SpikingSelfAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1, T=4):
        super().__init__()
        ...
        self.q_conv = IRConv1d(dim, dim, kernel_size=1, stride=1, bias=False)
        self.q_bn = nn.BatchNorm1d(dim)

        self.q_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy')
        self.k_conv = IRConv1d(dim, dim, kernel_size=1, stride=1, bias=False)
        self.k_bn = nn.BatchNorm1d(dim)

        self.k_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy')
        self.v_conv = IRConv1d(dim, dim, kernel_size=1, stride=1, bias=False)
        self.v_bn = nn.BatchNorm1d(dim)
        self.v_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy')
        
        self.attn_lif0 = MultiStepLIFNode(tau=2.0, v_threshold=1.0, v_reset=None, detach_reset=True, backend='cupy')

        self.attn_lif = MultiStepLIFNode(tau=2.0, v_threshold=0.5, detach_reset=True, backend='cupy')
        self.proj_conv = IRConv1d(dim, dim, kernel_size=1, stride=1)
        self.proj_bn = nn.BatchNorm1d(dim)

        self.lambda_attn = nn.Parameter(torch.ones(T, 1, num_heads, 1, 1), requires_grad=True)
        self.register_parameter("lambda_attn", self.lambda_attn)
    ...
    def forward(self, x):
        ...
        attn = self.attn_lif0(attn)
        x = (attn @ v) * 0.125 * self.lambda_attn
...
class RevSpikingTransformer(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, sr_ratio=1, T=4):
        super().__init__()
        self.attn = SpikingSelfAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
                                           attn_drop=attn_drop, proj_drop=drop, sr_ratio=sr_ratio)
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
    ...
    def forward(self, x0, x1):
        if x0 is None:
            ...
            x2 = self.attn(x1) + (x0 + x1) / 2
        x3 = self.mlp(x2) + (x1 + x2) / 2
        return x2, x3
...
class SpikingTokenizer(nn.Module):
    def __init__(self, img_size_h=128, img_size_w=128, patch_size=4, in_channels=2, embed_dims=256):
        ...
        self.block1_conv = IRConv2d(embed_dims // 8, embed_dims // 4, kernel_size=3, stride=1, padding=1, bias=False)
        self.block1_bn = nn.BatchNorm2d(embed_dims // 4)

        self.block2_lif = MultiStepLIFNode(tau=2.0, detach_reset=True, backend='cupy')
        self.block2_conv = IRConv2d(embed_dims // 4, embed_dims // 2, kernel_size=3, stride=1, padding=1, bias=False)
        ...
        self.block3_conv = IRConv2d(embed_dims // 2, embed_dims // 1, kernel_size=3, stride=1, padding=1, bias=False)
        ...
        self.block4_conv = IRConv2d(embed_dims, embed_dims, kernel_size=3, stride=1, padding=1, bias=False)
...
class vit_snn(nn.Module):
    def __init__(self,
        ...
        block = nn.ModuleList([RevSpikingTransformer(
            dim=embed_dims, num_heads=num_heads, mlp_ratio=mlp_ratios, qkv_bias=qkv_bias,
            qk_scale=qk_scale, drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[j],
            norm_layer=norm_layer, sr_ratio=sr_ratios, T=T)
        ...
        self.head2 = nn.Linear(embed_dims, num_classes) if num_classes > 0 else nn.Identity()
    ...
    def forward_features(self, x):
        ...
        x0 = None
        x1, (H, W) = patch_embed(x)
        for i, blk in enumerate(block):
            x0, x1 = blk(x0, x1)
        
        return x0.flatten(3).mean(3), x1.flatten(3).mean(3)
        

    def forward(self, x):
        x = (x.unsqueeze(0)).repeat(self.T, 1, 1, 1, 1)
        x0, x1 = self.forward_features(x)
        x0 = self.head(x0.mean(0))
        x1 = self.head2(x1.mean(0))
        
        return x0, x1

@register_model
def BESTFormer(pretrained=False, **kwargs):
    model = vit_snn(
        **kwargs
    )
